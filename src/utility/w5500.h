...
#ifndef ETHERNET5500_SRC_UTILITY_W5500_H_
#define ETHERNET5500_SRC_UTILITY_W5500_H_
...
class W5500Class {
 public:
  ...
  /**
   * @brief	This function is being used for copy the data form Receive
   * buffer of the chip to application buffer.
   *
   * It calculate the actual physical address where one has to read
   * the data from Receive buffer. Here also take care of the condition while it
   * exceed the Rx memory uper-bound of socket.
   */
  void read_data(SOCKET s, volatile uint16_t src, volatile uint8_t *dst,
                 uint16_t len);

  /**
   * @brief	 This function is being called by send() and sendto() function
   * also.
   *
  ...
  void send_data_processing_offset(SOCKET s, uint16_t data_offset,
   ...
   * and after copy the data from receive buffer update the Rx write pointer
   * register. User should read upper byte first and lower byte later to get
   * proper value.
   */
 ...
 private:
  ...
  static uint16_t write(uint16_t _addr, uint8_t _cb, const uint8_t *buf,
                        uint16_t len);
...
#define __GP_REGISTER_N(name, address, size)    \
 ...
 public:
  ...
  __GP_REGISTER8(
      PHYCFGR,
      0x002E);  // PHY Configuration register, default value: 0b 1011 1xxx
...
#undef __GP_REGISTER_N
 ...
 private:
  ...
  static inline uint16_t readSn(SOCKET _s, uint16_t _addr, uint8_t *_buf,
                                uint16_t len);
  static inline uint16_t writeSn(SOCKET _s, uint16_t _addr, uint8_t *_buf,
                                 uint16_t len);
...
uint16_t W5500Class::readSn(SOCKET _s, uint16_t _addr, uint8_t *_buf,
                            uint16_t _len) {
  uint8_t cntl_byte = (_s << 5) + 0x08;
  return read(_addr, cntl_byte, _buf, _len);
}

uint16_t W5500Class::writeSn(SOCKET _s, uint16_t _addr, uint8_t *_buf,
                             uint16_t _len) {
  uint8_t cntl_byte = (_s << 5) + 0x0C;
  return write(_addr, cntl_byte, _buf, _len);
}

void W5500Class::getGatewayIp(uint8_t *_addr) { readGAR(_addr); }

void W5500Class::setGatewayIp(uint8_t *_addr) { writeGAR(_addr); }

void W5500Class::getSubnetMask(uint8_t *_addr) { readSUBR(_addr); }

void W5500Class::setSubnetMask(uint8_t *_addr) { writeSUBR(_addr); }

void W5500Class::getMACAddress(uint8_t *_addr) { readSHAR(_addr); }

void W5500Class::setMACAddress(uint8_t *_addr) { writeSHAR(_addr); }

void W5500Class::getIPAddress(uint8_t *_addr) { readSIPR(_addr); }

void W5500Class::setIPAddress(uint8_t *_addr) { writeSIPR(_addr); }

void W5500Class::setRetransmissionTime(uint16_t _timeout) {
  writeRTR(_timeout);
}

uint16_t W5500Class::getRetransmissionTime() { return readRTR(); }

void W5500Class::setRetransmissionCount(uint8_t _retry) { writeRCR(_retry); }

uint8_t W5500Class::getRetransmissionCount() { return readRCR(); }

void W5500Class::setPHYCFGR(uint8_t _val) { writePHYCFGR(_val); }

uint8_t W5500Class::getPHYCFGR() { return readPHYCFGR(); }

#endif  // ETHERNET5500_SRC_UTILITY_W5500_H_
