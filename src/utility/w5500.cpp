...
void W5500Class::send_data_processing(SOCKET s, const uint8_t *data,
                                      uint16_t len) {
...
void W5500Class::read_data(SOCKET s, volatile uint16_t src,
...
uint16_t W5500Class::write(uint16_t _addr, uint8_t _cb, const uint8_t *_buf,
                           uint16_t _len) {
  SPI.beginTransaction(wiznet_SPI_settings);
  setSS();
  SPI.transfer(_addr >> 8);
