 ...
 * and set the port and wait for w5500 done it.
...
uint8_t listen(SOCKET s) {
  if (w5500.readSnSR(s) != SnSR::INIT) return 0;
...
int16_t recv(SOCKET s, uint8_t *buf, int16_t len) {
  ...
  if (ret == 0) {
    ...
    if (status == SnSR::LISTEN || status == SnSR::CLOSED ||
      ...
      // The remote end has closed its side of the connection, so this is the
      // eof state
      ret = 0;
    } else {
 ...
 */
uint16_t sendto(SOCKET s, const uint8_t *buf, uint16_t len, uint8_t *addr,
                uint16_t port) {
  uint16_t ret = 0;
  ...
  if (((addr[0] == 0x00) && (addr[1] == 0x00) && (addr[2] == 0x00) &&
    ...
    while ((w5500.readSnIR(s) & SnIR::SEND_OK) != SnIR::SEND_OK) {
      if (w5500.readSnIR(s) & SnIR::TIMEOUT) {
        ...
        w5500.writeSnIR(
            s, (SnIR::SEND_OK | SnIR::TIMEOUT)); /* clear SEND_OK & TIMEOUT */
        return 0;
      }
    }
...
/**
 * @brief	This function is an application I/F function which is used to
 * receive the data in other then TCP mode. This function is used to receive
 * UDP, IP_RAW and MAC_RAW mode, and handle the header as well.
 *
 * @return	This function return received data size for success else -1.
 */
uint16_t recvfrom(SOCKET s, uint8_t *buf, uint16_t len, uint8_t *addr,
                  uint16_t *port) {
  uint8_t head[8];
  uint16_t data_len = 0;
  uint16_t ptr = 0;
