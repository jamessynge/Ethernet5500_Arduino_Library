...
void DhcpClass::reset_DHCP_lease() {
  // zero out _dhcpSubnetMask, _dhcpGatewayIp, _dhcpLocalIp, _dhcpDhcpServerIp,
  // _dhcpDnsServerIp
...
int DhcpClass::request_DHCP_lease() {
  ...
  while (_dhcp_state != STATE_DHCP_LEASED) {
    ...
    if (result != 1 && ((millis() - startTime) > _timeout)) break;
  }
...
void DhcpClass::send_DHCP_MESSAGE(uint8_t messageType,
  ...
  if (_customHostname == 0) {
    ...
    buffer[17] = strlen(HOST_NAME) +
                 6;  // length of hostname + last 3 bytes of mac address
...
uint8_t DhcpClass::parseDHCPResponse(unsigned long responseTimeout,
  ...
  if (fixedMsg.op == DHCP_BOOTREPLY &&
      _dhcpUdpSocket.remotePort() == DHCP_SERVER_PORT) {
    ...
    while (_dhcpUdpSocket.available() > 0) {
      switch (_dhcpUdpSocket.read()) {
        ...
        case dhcpIPaddrLeaseTime:
          ...
          opt_len = _dhcpUdpSocket.read();
          _dhcpUdpSocket.read((uint8_t*)&_dhcpLeaseTime,
...
IPAddress DhcpClass::getLocalIp() { return IPAddress(_dhcpLocalIp); }

IPAddress DhcpClass::getSubnetMask() { return IPAddress(_dhcpSubnetMask); }

IPAddress DhcpClass::getGatewayIp() { return IPAddress(_dhcpGatewayIp); }

IPAddress DhcpClass::getDhcpServerIp() { return IPAddress(_dhcpDhcpServerIp); }

IPAddress DhcpClass::getDnsServerIp() { return IPAddress(_dhcpDnsServerIp); }
