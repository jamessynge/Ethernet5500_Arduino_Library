 ...
 */
#ifndef ETHERNET5500_SRC_ETHERNET5500_H_
#define ETHERNET5500_SRC_ETHERNET5500_H_
...
class EthernetClass {
 ...
 public:
  ...
  void setRstPin(uint8_t pinRST = 9);  // for WIZ550io or USR-ES1, must set
                                       // befor Ethernet.begin
  ...
  // be carefull of the MAX_SOCK_NUM, because in the moment it can't dynamicly
  // changed
...
#else
  // Initialize the Ethernet shield to use the provided MAC address and gain the
  // rest of the configuration through DHCP. Returns 0 if the DHCP configuration
  ...
  uint8_t duplex();  // returns duplex mode 0 = no link, 1 = Half Duplex, 2 =
                     // Full Duplex
  const char* duplexReport();  // returns duplex mode as a string

  void setRtTimeOut(
      uint16_t timeout = 2000);        // set the retransmission timout *100us
...
#endif  // ETHERNET5500_SRC_ETHERNET5500_H_
